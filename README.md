...
local introGUIEnabled = true
local introSongChoice = 3
local introSoundInstance = nil
local setIntroVisual = nil
local setIntroGUIVisual = nil
local setIntroSongVisual = nil
...
local INTRO_SONGS = {
    [1] = { url = "https://files.catbox.moe/43iq8z.mp3", file = "iblis_intro1.mp3" },
    [2] = { url = "https://files.catbox.moe/pg5ir9.mp3", file = "iblis_intro2.mp3" },
    [3] = { url = "https://files.catbox.moe/hg5cr4.mp3", file = "iblis_intro3.mp3" },
}

-- INTRO FUNCTIONS
local function playIntroMusic()
    if not introSoundEnabled then return end
    local song = INTRO_SONGS[introSongChoice] or INTRO_SONGS[3]
    task.spawn(function()
        local ok, data = pcall(function() return game:HttpGet(song.url) end)
        if ok and data then pcall(function() writefile(song.file, data) end) end
        if introSoundInstance then pcall(function() introSoundInstance:Destroy() end); introSoundInstance = nil end
...
local function stopIntroMusic()
    if introSoundInstance and introSoundInstance.IsPlaying then
        pcall(function() introSoundInstance:Stop() end)
    end
end

local function playIntroGUI()
    if not introGUIEnabled then return end
    local playerGui = LP:WaitForChild("PlayerGui")
    local oldGui = playerGui:FindFirstChild("IblisIntro")
    if oldGui then oldGui:Destroy() end
    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = "IblisIntro"
    screenGui.ResetOnSpawn = false
    screenGui.IgnoreGuiInset = true
    screenGui.DisplayOrder = 99999
    screenGui.Parent = playerGui
    local bgImage = Instance.new("ImageLabel")
    bgImage.Name = "IntroBackground"
    ...
    local title = createText("IBLIS HUB", 56, 0.45)
    local line = Instance.new("Frame")
    line.Size = UDim2.new(0, 200, 0, 3)
    line.Position = UDim2.new(0.5, 0, 0.52, 0)
    ...
    local subtitle = createText("discord.gg/SrBSCVyKH", 16, 0.57)
    local function fadeIn(object, property, duration)
        local tweenInfo = TweenInfo.new(duration or 0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        local tween = TS:Create(object, tweenInfo, {[property] = 0})
        tween:Play()
        return tween
    end
    local function fadeOut(object, property, duration)
        local tweenInfo = TweenInfo.new(duration or 0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        local tween = TS:Create(object, tweenInfo, {[property] = 1})
        tween:Play()
        return tween
    end
    task.wait(0.3)
    fadeIn(bgImage, "ImageTransparency", 1.0)
    fadeIn(overlay, "BackgroundTransparency", 1.0)
    task.wait(0.7)
    fadeIn(title, "TextTransparency", 0.8)
    task.wait(0.5)
    fadeIn(line, "BackgroundTransparency", 0.8)
    task.wait(0.3)
    fadeIn(subtitle, "TextTransparency", 0.8)
    task.wait(2.5)
    fadeOut(title, "TextTransparency", 0.8)
    fadeOut(line, "BackgroundTransparency", 0.8)
    fadeOut(subtitle, "TextTransparency", 0.8)
    fadeOut(bgImage, "ImageTransparency", 0.8)
    fadeOut(overlay, "BackgroundTransparency", 0.8)
    task.wait(0.9)
    pcall(function() screenGui:Destroy() end)
end

-- ANTI DIE FUNCTIONS
local function antiDieProcessHumanoid(hum, char)
    ...
    pcall(function()
        char:GetPropertyChangedSignal("BreakJoints"):Connect(function()
            if not antiDieEnabled then return end
            if char.BreakJoints then char.BreakJoints = false end
        end)
    end)
end
...
local function startAntiDie()
    ...
    if char then
        ...
        if hum then antiDieProcessHumanoid(hum, char) end
    end
    ...
    if antiDieCharConn then antiDieCharConn:Disconnect() end
    antiDieCharConn = LP.CharacterAdded:Connect(function(c)
        if not antiDieEnabled then return end
        task.wait(0.3)
        local h = c:FindFirstChildOfClass("Humanoid")
        if h then antiDieProcessHumanoid(h, c) end
    end)
    pcall(function()
...
function buildMobileButtons() 
    ...
    local g=Instance.new("ScreenGui");g.Name="IblisHubMobile";g.ResetOnSpawn=false;g.DisplayOrder=20;g.IgnoreGuiInset=true 
...
local function buildGui() 
    ...
    setAntiDieVisual=mkToggle(aimbotPage,"Anti Die",function(on) 
    ...
    mkSect(configPage,"Intro") 
    setIntroVisual=mkToggle(configPage,"Intro Music",function(on) 
        introSoundEnabled=on 
        if not on then stopIntroMusic() end 
        saveConfig() 
    end) 
    setIntroGUIVisual=mkToggle(configPage,"Intro GUI",function(on) 
        introGUIEnabled=on 
        saveConfig() 
    end) 
    do 
        local row=mkRow(configPage,32);mkLabel(row,"Intro Song") 
        local songBtn=Instance.new("TextButton",row) 
        songBtn.Size=UDim2.new(0,90,0,22);songBtn.Position=UDim2.new(1,-96,0.5,-11) 
        songBtn.BackgroundColor3=INP;songBtn.BorderSizePixel=0 
        songBtn.Text="SONG "..tostring(introSongChoice) 
        songBtn.TextColor3=WHITE;songBtn.Font=Enum.Font.SciFi;songBtn.TextSize=9;songBtn.ZIndex=5 
        Instance.new("UICorner",songBtn).CornerRadius=UDim.new(0,5) 
        Instance.new("UIStroke",songBtn).Color=STROKE 
        songBtn.Activated:Connect(function() 
            introSongChoice=introSongChoice+1 
            if introSongChoice>3 then introSongChoice=1 end 
            songBtn.Text="SONG "..tostring(introSongChoice) 
            saveConfig() 
        end) 
    end 
...
stealBarContainer.Size = UDim2.new(0, BAR_W, 0, BAR_H) 
stealBarContainer.Position = UDim2.new(0.5, -BAR_W/2, 1, -70) 
...
sbMain.Name = "MainPill"
sbMain.Size = UDim2.new(1, 0, 1, 0) 
sbMain.Position = UDim2.new(0, 0, 0, 0) 
sbMain.BackgroundColor3 = Color3.fromRGB(16, 16, 20)
sbMain.BackgroundTransparency = 0
sbMain.BorderSizePixel = 0 
sbMain.ZIndex = 2 
Instance.new("UICorner", sbMain).CornerRadius = UDim.new(1, 0)
...
sbBgImage.ImageTransparency = 0.35
sbBgImage.ZIndex = 1
Instance.new("UICorner", sbBgImage).CornerRadius = UDim.new(1, 0)
...
sbOverlay.BackgroundTransparency = 0.5
sbOverlay.BorderSizePixel = 0
sbOverlay.ZIndex = 1
Instance.new("UICorner", sbOverlay).CornerRadius = UDim.new(1, 0)

local sbBorder = Instance.new("UIStroke", sbMain) 
sbBorder.Color = Color3.fromRGB(255, 255, 255) 
sbBorder.Thickness = 1.2 
sbBorder.Transparency = 0.55
sbBorder.ApplyStrokeMode = Enum.ApplyStrokeMode.Border 
sbBorder.ZIndex = 4

local stealPill = Instance.new("Frame", sbMain)
stealPill.Name = "StealPill"
stealPill.Size = UDim2.new(0, STEAL_W, 1, -8)
stealPill.Position = UDim2.new(0, 5, 0, 4)
stealPill.BackgroundColor3 = Color3.fromRGB(24, 24, 28)
stealPill.BackgroundTransparency = 0
...
local progressFill = Instance.new("Frame", stealPill) 
...
Instance.new("UICorner", progressFill).CornerRadius = UDim.new(0, PILL_RADIUS)

local progressPct = Instance.new("TextLabel", stealPill) 
...
local fpsPillX = STEAL_W + 12
local fpsPillW = 78
local fpsPill = Instance.new("Frame", sbMain)
fpsPill.Name = "FpsPill"
fpsPill.Size = UDim2.new(0, fpsPillW, 1, -14)
fpsPill.Position = UDim2.new(0, fpsPillX, 0, 7)
fpsPill.BackgroundColor3 = Color3.fromRGB(24, 24, 28)
fpsPill.BackgroundTransparency = 0.15
fpsPill.BorderSizePixel = 0
fpsPill.ZIndex = 3
Instance.new("UICorner", fpsPill).CornerRadius = UDim.new(1, 0)

local pingBarLbl = Instance.new("TextLabel", fpsPill) 
pingBarLbl.Size = UDim2.new(1, -8, 1, 0) 
pingBarLbl.Position = UDim2.new(0, 4, 0, 0) 
pingBarLbl.BackgroundTransparency = 1 
pingBarLbl.Text = "60 FPS" 
pingBarLbl.TextColor3 = Color3.fromRGB(220, 220, 220) 
pingBarLbl.Font = Enum.Font.GothamBold 
pingBarLbl.TextSize = 11 
pingBarLbl.TextXAlignment = Enum.TextXAlignment.Center 
pingBarLbl.ZIndex = 4

local pingPillX = fpsPillX + fpsPillW + 6
local pingPillW = 90
local pingPill = Instance.new("Frame", sbMain)
pingPill.Name = "PingPill"
pingPill.Size = UDim2.new(0, pingPillW, 1, -14)
pingPill.Position = UDim2.new(0, pingPillX, 0, 7)
pingPill.BackgroundColor3 = Color3.fromRGB(24, 24, 28)
pingPill.BackgroundTransparency = 0.15
pingPill.BorderSizePixel = 0
pingPill.ZIndex = 3
Instance.new("UICorner", pingPill).CornerRadius = UDim.new(1, 0)

local pingLbl = Instance.new("TextLabel", pingPill) 
pingLbl.Size = UDim2.new(1, -8, 1, 0) 
pingLbl.Position = UDim2.new(0, 4, 0, 0) 
...
local dotBg = Instance.new("Frame", sbMain)
dotBg.Name = "DotBackground"
dotBg.Size = UDim2.fromOffset(28, 28)
dotBg.Position = UDim2.new(1, -34, 0.5, -14)
dotBg.BackgroundColor3 = Color3.fromRGB(24, 24, 28)
dotBg.BackgroundTransparency = 0.15
...
task.spawn(function() 
    ...
    while task.wait(0.3) do 
        ...
        pingBarLbl.Text = string.format("%d FPS", math.floor(fpsAvg + 0.5)) 
...
RunService.RenderStepped:Connect(function(dt) 
    ...
    if dot then
        if f > 0.01 then
            dot.BackgroundColor3 = Color3.fromRGB(60, 150, 255)
        else
            dot.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        end
    end
...
UIS.InputChanged:Connect(function(inp) 
    ...
    if inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch then 
        ...
        stealBarContainer.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y) 
    end 
end) 
...
local function loadConfigKeys() 
    ...
    if cfg.introSongChoice then introSongChoice=tonumber(cfg.introSongChoice) or 3 end 
    if cfg.introGUIEnabled ~= nil then introGUIEnabled=cfg.introGUIEnabled==true end 
...
local function loadConfigState() 
    ...
    task.spawn(function() 
        ...
        if cfg.antiDie then startAntiDie();if setAntiDieVisual then setAntiDieVisual(true) end end 
        ...
        if setIntroVisual then setIntroVisual(introSoundEnabled) end 
        if setIntroGUIVisual then setIntroGUIVisual(introGUIEnabled) end 
    end) 
end 
loadConfigKeys() 

-- Lancer l'intro AVANT le GUI principal
if introSoundEnabled then playIntroMusic() end 
if introGUIEnabled then playIntroGUI() end 
